# 職務経歴書

## Agenda

- [基本情報](#基本情報) 
- [学歴・職歴](#学歴職歴) 
- [習得・経験した技術](#習得経験した技術) 
    - [プログラミング・実装経験](#プログラミング実装経験) 
    - [デザイン経験](#デザイン経験) 
    - [その他経験](#その他経験) 
- [スキル](#スキル) 
    - [デザイン](#デザイン) 
    - [プログラム言語](#プログラム言語) 
- [職務経歴](#職務経歴) 
- [自己PR](#自己pr) 
- [保有資格](#保有資格)
- [受賞歴](#受賞歴)

## 基本情報

**Shinichi Kogiso(kgsi)**  

|媒体|リンク|
|---|-----|
|web|[Aircolor](http://aircolor.org)|
|blog|[Aircolor memolog](http://aircolor.hatenablog.com)|
|twitter|[https://twitter.com/kgsi](https://twitter.com/kgsi)|
|qiita|[https://qiita.com/kgsi](https://qiita.com/kgsi)|
|dribbble|[https://dribbble.com/kgsi](https://dribbble.com/kgsi)|
|mail|[Gmail](kuusai1998@gmail.com)|

## 学歴・職歴
|年月|職歴|
|---|-----|
|2007.03|文星芸術大学 美術学部 デジタルグラフィックスコース 卒業|
|2007.04|都内コンテンツ制作会社 デザイナーとして入社|
|2008.03|都内コンテンツ制作会社 退社|
|2008.04|都内自社サービス運営会社 Webデザイナーとして入社|
|2009.07|都内自社サービス運営会社 退社|
|2009.08|株式会社イー・バード Webデザイナーとして入社|
|2014.08|株式会社イー・バード 所属部署変更 エンジニア・ディレクターへの業務転換　現在に至る|

## 習得・経験した技術

### プログラミング・実装経験

- Browserify, Gulp, Webpack等のビルドツールを利用した開発
- jQuery、CreateJS、D3.jsを利用したリッチWebコンテンツの開発
- Electronフレームワークを使ったデスクトップ・インスタレーションアプリの開発
- Cordovaフレームワークを使ったハイブリッドアプリの構築・リリース（iOSに特化）
- Vue.jsを使ったWebアプリケーションの開発
- LAMP環境でのPHP + MySQLを使ったWebアプリケーション、Webサイトの構築
- WordPressによるCMS実装、簡易バックエンド環境の構築
- HTML5, CSS3, JavaScript を使ったWebフロントエンドの制作
- Bitbucket, SourceTreeを使ったGit管理・開発
- AWS, Azure, Firebaseを使ったバックエンド構築（ライトな開発）

### デザイン・UI・UX制作経験

- 各種デザインツールを使ったWebサイト・アプリケーションデザイン制作
- 各種デザインツールを使ったグラフィックデザイン制作
- Prott、inVision等のプロトタイピングツールを使ったモックアップ作成
- ワイヤーフレームやプロトタイピングを通しての提案、ディスカッション経験

### その他経験
- 全体スケジュール管理、及びクライアント応対業務
- 空撮用ドローンによる撮影業務
- 海外赴任経験あり（タイ支社・赴任期間：合計約3ヶ月）


## スキル

### デザイン
- Adobe Photoshop
- Adobe Illustrator
- Sketch

### プログラム言語
#### 実務経験あり
- HTML(EJS, pug)
- CSS
- Sass
- JavaScript
- TypeScript(angular,ionicでのみ)
- PHP
- Python
- C#(Unity)
- Markdown

#### 趣味レベル
- Processing(P5.js)
- PostCSS

### フレームワーク・ライブラリ利用経験
#### 実務経験あり
- Vue.js
- OnsenUI
- Electron
- jQuery
- CreateJS
- D3.js
- Ionic
- Cordova
- Phonegap

## 職務経歴

### 2009.08 - 現在: 株式会社イー・バード

#### 経験職務
- Webデザイナー
- エンジニア
- Webディレクター

#### Webデザイナー : 2009.08 - 2014.08
- 受託案件Webサイトのデザイン・HTMLコーディングをメインに担当
- 受託案件Webサイトの更新・修正作業
- WordPress・MovableTypeを使ったCMS構築
- 名刺・チラシ・カタログなどのデザイン制作

#### エンジニア/Webデザイナー/Webディレクター : 2014.08 - 現在
- ネイティブアプリ(iOS, Android)のデザイン制作
- ハイブリッドアプリ(iOS, Android)制作のアプリ開発、デザイン制作
- Unity(2D)を使ったインスタレーションアプリの開発
- 受託案件Webサイトのデザイン・HTMLコーディン・WordPress・MT実装は引き続き兼務
- レスポンシブWebサイトのマークアップ、アニメーション実装
- ドローン空撮業務の兼務
- スケジュール管理、及び顧客対応業務

※作業比割合としてエンジニア作業:5, デザイン作業:3, ディレクション業務:2 ほど


### 2008.04 - 2009.07: 都内自社サービス運営会社

#### Webデザイナー : 2008.04 - 2009.07
- 自社Webサービスのデザイン制作
- WordPress使ったCMS構築
- 商業施設のチラシ・ポスター制作

### 2007.04 - 2008.03: 都内コンテンツ制作会社

#### グラフィックデザイナー : 2007.04 - 2008.03
- 受託案件のDVDパッケージデザイン制作
- 受託案件のDVDコンテンツ（Flash）制作
- 受託案件のチラシ・ポスター制作
- Scenaristを使ったDVDオーサリング作業

## 自己PR
所属している会社の特徴で代理店を通さず、直で受注する案件が多くを占めるため、必然的にWebサイトやアプリケーションのデザイン/UI設計から、最終的な実装まで全ての工程で携わり、ほぼ一通りの制作経験を積むことができました。今現在は顧客対応まで対応範囲が広がり、ワンストップに業務を担当しています。

また開発部に所属してからは、実装ルールがバラバラだったシステム構築のルールを統一する目的で、HTMLマークアップ仕様書や[WordPressの構築設計ガイドライン](https://qiita.com/kgsi/items/65ee2d7814e5cfa199d2)などをまとめ、社内レベル、及び社外への発注レベルを一定値以上に底上げする、などの啓蒙活動も行っています。

## 保有資格
- 第三級陸上特殊無線技士 (2017〜)
- JAL Global Club (2014〜)
- ANA Super Flyers Card (2017〜)

## 受賞歴
- [はてなブログ テーマコンテスト 最優秀賞](http://staff.hatenablog.com/entry/2013/05/27/160012)

## 登壇経験
- [「Webのこれからを語ろう」Ionic Meetup Tokyo #4 LT](https://speakerdeck.com/kgsi/ben-fan-huan-jing-wofen-kerusinpurunafang-fa-with-ionic)
